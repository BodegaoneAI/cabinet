...
export function AppShell() {
  ...
  // When running inside Bodega One's agent workspace panel, start compact:
  // sidebar collapsed, AI panel collapsed. Cabinet detects embedded mode
  // via window.isCabinetEmbedded set by cabinet-preload.js.
  const isEmbedded = typeof window !== 'undefined' && !!(window as Window & { isCabinetEmbedded?: boolean }).isCabinetEmbedded;
  const [embeddedInitDone, setEmbeddedInitDone] = useState(!isEmbedded);
  useEffect(() => {
    if (!isEmbedded || embeddedInitDone) return;
    setSidebarCollapsed(true);
    setAiPanelCollapsed(true);
    setEmbeddedInitDone(true);
  }, [isEmbedded, embeddedInitDone, setSidebarCollapsed, setAiPanelCollapsed]);
